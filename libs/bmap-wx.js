                ...
                success(data) {
                  ...
                  //  console.log("res:"+res);
                    if (res["status"] === 0) {
						...
						//console.log(poiArr[0].detail_info.comment_num);
                       
                        for (let i = 0; i < poiArr.length; i++) {
                            outputRes["wxMarkerData"][i] = {
                                id: i,
                                latitude: poiArr[i]["location"]["lat"],   //纬度
                                longitude: poiArr[i]["location"]["lng"],  //经度
                                title: poiArr[i]["name"],                 //名称
                                iconPath: otherparam["iconPath"],
                                iconTapPath: otherparam["iconTapPath"],
                                uid: poiArr[i]["uid"],           //唯一编码
                                address: poiArr[i]["address"],           //地址
                                telephone: poiArr[i]["telephone"],       //电话
                                alpha: otherparam["alpha"],
                                width: otherparam["width"],
                                height: otherparam["height"],
                                distance:poiArr[i].detail_info.distance ,  //距离
                                overall_rating:poiArr[i].detail_info.overall_rating ,  //总体评分
                                price:poiArr[i].detail_info.price ,  //poi商户的价格
                                distance:poiArr[i].detail_info.distance ,  //距离中心点的距离，圆形区域检索时返回
                                environment_rating:poiArr[i].detail_info.environment_rating ,  //环境评分
                                service_rating:poiArr[i].detail_info.service_rating ,  //服务评分
                                groupon_num:poiArr[i].detail_info.groupon_num ,  //团购数
                                detail_url:poiArr[i].detail_info.detail_url ,  //poi的详情页
            ...
            wx.request({
                ...
                success(data) {
                    ...
                    if (res["status"] === 0) {
                        ...
                        // originalData为百度接口返回的原始数据
                        // wxMarkerData为小程序规范的marker格式
						let addr=poiObj["addressComponent"];
                        let outputRes = {};
                        outputRes["originalData"] = res;
                        outputRes["wxMarkerData"] = [];    
